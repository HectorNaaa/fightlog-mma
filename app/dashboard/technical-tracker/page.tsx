    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-stone-text mt-1">Rate and analyze your techniques</p>
        </div>
        {isIntermediate ? (
          <Button onClick={openNew}>+ Add Technique</Button>
        ) : (
          <div className="text-xs text-stone-text border border-stone-border px-3 py-1.5 rounded-sm">Intermediate+ unlocks full tracker</div>
        )}
      </div>
      ...
      {!isIntermediate ? (
        <BeginnersView tab={tab} presets={presets} />
      ) : (
        <Card>
          <CardBody className="p-0 overflow-x-auto">
            {filtered.length === 0 ? (
              <div className="p-10 text-center text-stone-text text-sm">
                No {tab} techniques yet. Add your first one.
              </div>
            ) : (
                ...
                <tbody>
                  {filtered.map((t) => (
                    <tr key={t.id}>
                      ...
                      <td>{t.videoUrl ? <a href={t.videoUrl} target="_blank" className="text-amber text-xs underline">Watch</a> : "—"}</td>
                      <td>
                        <div className="flex gap-1">
                          <Button variant="ghost" size="sm" onClick={() => openEdit(t)}>Edit</Button>
                          <Button variant="danger" size="sm" onClick={() => del(t.id)}>Del</Button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editing ? "Edit Technique" : "Add Technique"} className="max-w-2xl">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2 flex flex-col gap-1">
            <label className="text-xs font-semibold uppercase tracking-wider text-stone-text">Technique Name</label>
            <input
              list="tech-presets"
              value={form.name}
              onChange={f("name")}
              ...
              placeholder="Select or type technique name"
            />
            <datalist id="tech-presets">
              {presets.map((p) => <option key={p} value={p} />)}
            </datalist>
          </div>
          <Input label="Setup" value={form.setup ?? ""} onChange={f("setup")} placeholder="e.g. Jab to close range" />
          <Input label="Counter" value={form.counter ?? ""} onChange={f("counter")} placeholder="e.g. Step off-angle" />
          <Input label="Common Mistake" value={form.commonMistake ?? ""} onChange={f("commonMistake")} />
          <Input label="Success Rate %" type="number" min={0} max={100} value={form.successRate ?? ""} onChange={f("successRate")} />
          <div className="flex flex-col gap-1 sm:col-span-2">
            <label className="text-xs font-semibold uppercase tracking-wider text-stone-text">Confidence (1–10): {form.confidence ?? 0}</label>
            <input type="range" min={1} max={10} value={form.confidence ?? 5} onChange={f("confidence")} />
          </div>
          <Input label="Video URL (optional)" value={form.videoUrl ?? ""} onChange={f("videoUrl")} placeholder="https://..." className="sm:col-span-2" />
  ...
  return (
    <div>
      <div className="mb-4 p-3 border border-navy/30 bg-navy/10 rounded-sm text-xs text-navy-light">
        <span className="font-bold uppercase">Beginner Mode</span> — Focus on fundamentals. Upgrade to Intermediate to unlock full technique tracking.
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {presets.slice(0, 12).map((name) => (
          <div key={name} className="bg-bg-card border border-stone-border rounded-sm px-4 py-3">
            ...
            <div className="text-xs text-stone-text mt-1">Study this technique</div>
          </div>
        ))}
      </div>
    </div>
