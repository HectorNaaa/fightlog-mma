...
export async function POST(
  ...
  try {
    ...
    const existing = await prisma.techniqueSave.findUnique({
      where: { nodeId_userId: { nodeId: id, userId: me.userId } },
    });

    if (existing) {
      await prisma.techniqueSave.delete({ where: { id: existing.id } });
      return NextResponse.json({ saved: false });
    }

    await prisma.techniqueSave.create({
    ...
    await prisma.activityEvent.create({
      data: {
        userId: me.userId,
        eventType: "knowledge.node.saved",
        message: `Saved ${node.title}`,
        metadata: { nodeId: id },
      },
    });
    ...
    console.error("[technique-nodes/save] Internal error", error);
